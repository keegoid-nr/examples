...
- [New Relic APM + Serverless Convergence: Python Demo Project](#new-relic-apm--serverless-convergence-python-demo-project)
  ...
  - [Verification](#verification)
    ...
    - [Step 3: Verify Log Metrics](#step-3-verify-log-metrics)
  - [Clean Up](#clean-up)
...
APM + Serverless Convergence unifies the monitoring of serverless functions and traditional services within New Relic APM. This demo showcases standard instrumentation, including custom attributes and log-based metrics.
...
This project demonstrates how the New Relic Python agent automatically captures logging metrics. By using the standard Python `logging` library, the agent intercepts log calls and reports them as the `apm.service.logging.lines` metric.

In [app.py](file:///Users/kmullaney/repos/examples/lambda/sam/python/apm-mode/src/app.py), the agent hooks into the standard logging module to record these counts:

1. **Logger Configuration**: The standard `logging` library is configured.
2. **Log Interception**: Calls like `logger.info()` are intercepted by the agent.
3. **Metric Reporting**: The counts are reflected in the `apm.service.logging.lines` metric with the appropriate `level` attribute.

This allows you to monitor log volumes and levels (INFO, DEBUG, ERROR, etc.) directly within APM metrics.
...
### Step 3: Verify Log Metrics

You can verify that your log calls are being captured as metrics using NRQL:

1. Go to **Query Builder** in New Relic.
2. Run the following query to see the log line counts faceted by severity:

    ```sql
    SELECT count(apm.service.logging.lines) FROM Metric WHERE entity.name = 'Kmullaney-APM-Demo-Python312' FACET severity TIMESERIES
    ```

3. These metrics are automatically generated by the Python agent's instrumentation of the standard `logging` library, providing a high-level view of your function's logging activity.
